...
upper_limit_deg = 180
import math
lower_limit_rad = lower_limit_deg
upper_limit_rad = upper_limit_deg
...
def main():
    ...
    # Set limits in DEGREES (not radians)
    lower_limit = 30.0
    upper_limit = 180.0
    ...
    carb.log_info(f"Joint limits set from {lower_limit}° to {upper_limit}° and enabled PhysX limits.")

    # Apply DriveAPI with position drive (enforces limits)
    drive_api = UsdPhysics.DriveAPI.Apply(joint_prim, "angular")
    drive_api.CreateTypeAttr().Set("position")
    target_angle = 45.0  # degrees
    drive_api.CreateTargetPositionAttr().Set(target_angle)
    drive_api.CreateStiffnessAttr().Set(1000.0)
    drive_api.CreateDampingAttr().Set(100.0)
    drive_api.CreateMaxForceAttr().Set(5000.0)
    carb.log_info(f"Applied DriveAPI: position drive with target {target_angle}°.")
    ...
    if not physx:
        carb.log_error("Failed to get PhysX interface!")
        sys.exit(1)

    physx.start_simulation()
    carb.log_info("Started PhysX simulation")

    for _ in range(300):
        physx.update(1.0 / 60.0, time.time())
        time.sleep(1.0 / 60.0)

    physx.stop_simulation()
    carb.log_info("Stopped PhysX simulation")
...
if rod is not None:
    ...
    dc.apply_body_torque(rod, (0.0, 0.0, 10.0), False)
